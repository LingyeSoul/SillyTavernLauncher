...
import { TRAY_CALLBACK_MESSAGE } from '../services/tray'

let running: RunningApp | null = null

/** 阶段种子：三键齐/差一键（tray 参数化），外加镜像离线种子防测速等待 */
function startSilentApp(tray: boolean): Promise<RunningApp> {
  return startApp({
    dirPrefix: 'stl-silent-verify',
    seed: {
      autostart: true,
...
try {
  ...
  running = await startSilentApp(true)
  ...
  running = await startSilentApp(false)
